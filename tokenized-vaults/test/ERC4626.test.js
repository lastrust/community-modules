...
contract("ERC4626", function (accounts) {
  ...
  beforeEach(async function () {
    ...
    await this.token.approve(this.vault.address, constants.MAX_UINT256, {
      from: holder,
    });
  ...
  describe("empty vault: no assets & no shares", function () {
    ...
    it("deposit", async function () {
      ...
      expect(
        await this.vault.previewDeposit(parseToken(1))
      ).to.be.bignumber.equal(parseShare(1));

      const { tx } = await this.vault.deposit(parseToken(1), recipient, {
        from: holder,
      });
    ...
    it("mint", async function () {
      expect(await this.vault.maxMint(holder)).to.be.bignumber.equal(
        constants.MAX_UINT256
      );
      ...
      const { tx } = await this.vault.mint(parseShare(1), recipient, {
        from: holder,
      });
    ...
    it("withdraw", async function () {
      ...
      const { tx } = await this.vault.withdraw("0", recipient, holder, {
        from: holder,
      });
    ...
    it("redeem", async function () {
      ...
      const { tx } = await this.vault.redeem("0", recipient, holder, {
        from: holder,
      });
  ...
  describe("partially empty vault: assets & no shares", function () {
    ...
    it("status", async function () {
      expect(await this.vault.totalAssets()).to.be.bignumber.equal(
        parseToken(1)
      );
    });
  ...
  describe("partially empty vault: shares & no assets", function () {
    ...
    it("redeem", async function () {
      expect(await this.vault.maxRedeem(holder)).to.be.bignumber.equal(
        parseShare(1)
      );
      expect(
        await this.vault.previewRedeem(parseShare(1))
      ).to.be.bignumber.equal("0");

      const { tx } = await this.vault.redeem(parseShare(1), recipient, holder, {
        from: holder,
      });
  ...
  describe("full vault: assets & shares", function () {
    ...
    it("deposit", async function () {
      ...
      expect(
        await this.vault.previewDeposit(parseToken(1))
      ).to.be.bignumber.equal(parseShare(100));
    ...
    it("withdraw", async function () {
      ...
      expect(
        await this.vault.previewWithdraw(parseToken(1))
      ).to.be.bignumber.equal(parseShare(100));
    ...
    it("withdraw with approval", async function () {
      ...
      await this.vault.withdraw(parseToken(1), recipient, holder, {
        from: spender,
      });
    });
    ...
    it("redeem", async function () {
      ...
      expect(
        await this.vault.previewRedeem(parseShare(100))
      ).to.be.bignumber.equal(parseToken(1));

      const { tx } = await this.vault.redeem(
        parseShare(100),
        recipient,
        holder,
        { from: holder }
    ...
    it("redeem with approval", async function () {
      ...
      await this.vault.redeem(parseShare(100), recipient, holder, {
        from: spender,
      });
    });
  });
  ...
  it("multiple mint, deposit, redeem & withdrawal", async function () {
    ...
    // 1. Alice mints 2000 shares (costs 2000 tokens)
    {
      ...
      expect(await this.vault.previewDeposit(2000)).to.be.bignumber.equal(
        "2000"
      );
    ...
    // 2. Bob deposits 4000 tokens (mints 4000 shares)
    {
      ...
      expect(await this.vault.previewDeposit(4000)).to.be.bignumber.equal(
        "4000"
      );
      expect(await this.vault.balanceOf(user1)).to.be.bignumber.equal("2000");
    ...
    {
      const { tx } = await this.vault.redeem(1333, user1, user1, {
        from: user1,
      });
      expectEvent.inTransaction(tx, this.vault, "Transfer", {
        from: user1,
    ...
    {
      const { tx } = await this.vault.withdraw(2929, user2, user2, {
        from: user2,
      });
      expectEvent.inTransaction(tx, this.vault, "Transfer", {
        from: user2,
    ...
    {
      const { tx } = await this.vault.withdraw(3643, user1, user1, {
        from: user1,
      });
      expectEvent.inTransaction(tx, this.vault, "Transfer", {
        from: user1,
    ...
    {
      const { tx } = await this.vault.redeem(4392, user2, user2, {
        from: user2,
      });
      expectEvent.inTransaction(tx, this.vault, "Transfer", {
        from: user2,
