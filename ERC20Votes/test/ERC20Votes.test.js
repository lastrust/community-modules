...
const ERC20Votes = artifacts.require("ERC20VotesMock");
...
contract("ERC20Votes", function (accounts) {
  ...
  beforeEach(async function () {
    ...
    this.token = await ERC20Votes.new(name, symbol);
  });
  ...
  it("recent checkpoints", async function () {
    ...
    // recent
    expect(
      await this.token.getPastVotes(holder, block - 1)
    ).to.be.bignumber.equal("5");
    // non-recent
    expect(
      await this.token.getPastVotes(holder, block - 6)
  ...
  describe("set delegation", function () {
    ...
    describe("with signature", function () {
      const delegator = wallet["default"].generate();
