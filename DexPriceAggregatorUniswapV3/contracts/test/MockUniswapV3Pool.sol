...
contract MockUniswapV3Pool {
    ...
    function setObservations(
        uint32[] calldata _blockTimestamps,
        int56[] calldata _tickCumulatives
    ) external {
        require(
    ...
    function _calculateTickCumulative(uint32 _secondsAgo)
        private
        view
        returns (int56 tickCumulative)
    {
        ...
        for (uint16 ii; ii < currentObsCardinality; ++ii) {
            uint16 obsIndex = _prevObservationIndex(
                ii,
                currentObsIndex,
                currentObsCardinality
            );
            ...
            if (_lte(currentTimestamp, obs.blockTimestamp, target)) {
                if (obs.blockTimestamp == target) {
                    ...
                    uint16 priorIndex = _prevObservationIndex(
                        1,
                        currentObsIndex,
                        currentObsCardinality
                    );
                    ...
                    int24 tick = _calculateTickValueFromObservations(
                        obs,
                        priorObs
                    );
                    ...
                    int24 nextTick = _calculateTickValueFromObservations(
                        nextObs,
                        obs
                    );
                    uint32 delta = target - obs.blockTimestamp;
        ...
        revert(
            "MockUniswapV3Pool#_calculateTickCumulative could not find matching observation"
        );
    }
    ...
    function _calculateTickValueFromObservations(
        Observation memory _new,
        Observation memory _old
    ) private pure returns (int24 tick) {
        uint32 delta = _new.blockTimestamp - _old.blockTimestamp;
        return
            int24(
                (_new.tickCumulative - _old.tickCumulative) /
                    SafeCast.toInt56(SafeCast.toInt256(delta))
            );
    }
