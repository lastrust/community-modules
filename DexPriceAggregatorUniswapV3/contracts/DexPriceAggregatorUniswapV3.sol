...
contract DexPriceAggregatorUniswapV3 is IDexPriceAggregator, Ownable {
    ...
    function fetchCurrentTicks(address _pool, uint32 _twapPeriod)
        public
        view
    ...
    function _fetchAmountCrossingPools(
        ...
        address pool1 = _getPoolForRoute(
            PoolAddress.getPoolKey(_tokenIn, weth, defaultPoolFee)
        );
        address pool2 = _getPoolForRoute(
            PoolAddress.getPoolKey(_tokenOut, weth, defaultPoolFee)
        );

        int24 spotTick1 = OracleLibrary.getBlockStartingTick(pool1);
        int24 spotTick2 = OracleLibrary.getBlockStartingTick(pool2);
        uint256 spotAmountOut = _getQuoteCrossingTicksThroughWeth(
            _tokenIn,
            _amountIn,
            _tokenOut,
            spotTick1,
        ...
        uint256 twapAmountOut = _getQuoteCrossingTicksThroughWeth(
            _tokenIn,
            _amountIn,
            _tokenOut,
            twapTick1,
    ...
    function _getQuoteCrossingTicksThroughWeth(
        ...
        uint256 ethAmountOut = OracleLibrary.getQuoteAtTick(
            _tick1,
            SafeCast.toUint128(_amountIn),
            _tokenIn,
            weth
        ...
        return
            OracleLibrary.getQuoteAtTick(
                _tick2,
                SafeCast.toUint128(ethAmountOut),
                weth,
                _tokenOut
            );
    }
    ...
    function _getPoolForRoute(PoolAddress.PoolKey memory _poolKey)
        internal
        view
        returns (address pool)
    {
    ...
    function _identifyRouteFromPoolKey(PoolAddress.PoolKey memory _poolKey)
        internal
        pure
        returns (bytes32 id)
    {
    ...
    function _getOverriddenPool(PoolAddress.PoolKey memory _poolKey)
        internal
        view
        returns (address pool)
    {
